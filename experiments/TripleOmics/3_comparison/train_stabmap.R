...
run <- function(data_fn, res_fn, use_pseduo = TRUE, K = 30, seed = 0) {
  ...
  for (bi in dat$batch_names) {
    ...
    if (!is.null(met)) {
      rownames(met) <- paste0(bi, "_", 1:nrow(met))
      colnames(met) <- paste0("met_", 1:ncol(met))
    }
  ...
  grid_pp <- list()
  for (omic_name_i in c("atac", "rna", "met")) {
  ...
  for (batch_name_i in names(grid)) {
    grid_omic_i <- list()
    for (omics_name_i in c("atac", "rna", "met")) {
